...
async function create() {
  ...
  this.label_1 = createLabel.call(
    this,
    circle.x,
    circle.y + circle.radius + 10,
    COLOR_C1,
  );
  this.label_2 = createLabel.call(
    this,
    circle.x,
    circle.y - circle.radius - 10,
    COLOR_C2,
  );
  this.label_price = createLabel.call(
    this,
  ...
  this.input.on("pointermove", (pointer) => {
    this.coordLabel.setText(
      `(${((pointer.x / this.cameras.main.width) * 1000).toFixed(0)}, ${(1000 - (pointer.y / this.cameras.main.width) * 1000).toFixed(0)})`,
    );
...
function pointerMoveHandler(pointer) {
  ...
  if (
    !this.game_state.do_trade &&
    this.isMousePressed &&
    currentTime - this.lastPrintTime >= 50
  ) {
...
function setEntitlements({ x, y }) {
  ...
  this.game_state.people = [
    createIndividual([x, y], [0.25, 0.75], "c1", COLOR_C1_N, (a) => [
      (a[0] * this.cameras.main.width) / DENSITY,
      ((DENSITY - a[1]) * this.cameras.main.height) / DENSITY,
    ]),
    createIndividual(
      [1000 - x, 1000 - y],
      [0.75, 0.25],
      ...
      COLOR_C2_N,
      (a) => [
        ((DENSITY - a[0]) * this.cameras.main.width) / DENSITY,
        (a[1] * this.cameras.main.height) / DENSITY,
      ],
    ),
  ];
...
function setEntitlementsPtr(pointer) {
  setEntitlements.call(this, {
    x: Math.trunc(1000 * (pointer.x / this.cameras.main.width)),
    y: 1000 - Math.trunc((1000 * pointer.y) / this.cameras.main.height),
  });
}
...
function update() {
  if (this.graphics) {
    ...
    this.game_state.people.forEach(({ individual, translate }) => {
      this.graphics.lineStyle(1, individual.getColor(), 0.2);
    ...
    if (this.game_state.do_trade) {
      ...
      const diffs =
        history.length > 1
          ? [
            history[history.length - 2][0] - this.circle.x,
            history[history.length - 2][1] - this.circle.y,
          ]
          : [0, 0];
      const wrap_up = () => {
        ...
        const label = createLabel.call(
          this,
          circle.x,
          circle.y,
          BUTTON_COLOR,
          `${this.label_price_accum.text}`,
        );
    ...
    if (this.game_state.history.length > 1) {
      ...
      for (let i = 1; i < this.game_state.history.length - 1; i++) {
        ...
        this.graphics.moveTo(
          this.game_state.history[i][0],
          this.game_state.history[i][1],
        );
        this.graphics.lineTo(
          this.game_state.history[i + 1][0],
          this.game_state.history[i + 1][1],
        );
        this.graphics.strokePath();
      }
    }
    ...
    if (this.game_state.frame_count % 10 === 0)
      this.game_state.on_off = 1 - this.game_state.on_off;

    if (
      this.game_state.random_draw &&
      !this.game_state.do_trade &&
      Date.now() - this.last_interaction > 1000
    ) {
      this.game_state.history = [];
      this.game_state.bids = [];
      this.label_price.visible = true;
